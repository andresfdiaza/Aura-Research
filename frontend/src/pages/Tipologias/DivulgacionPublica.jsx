...
export default function DivulgacionPublica() {
  ...
  const filtered = React.useMemo(() => {
    let result = resultados.filter(r => {
      ...
      return tipologia === 'divulgación pública de la ciencia';
    });
    ...
    console.log('Filtrados (Divulgación Pública de la Ciencia):', result.length);
  ...
  const handleDownloadCSV = async () => {
    try {
      ...
      link.setAttribute('download', `divulgacion_publica_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden">
      ...
      <div className="container mx-auto flex-1 flex flex-col">
        ...
        <div className="max-w-7xl w-full flex flex-col gap-8">
          <div className="flex w-full flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <h1 className="text-2xl sm:text-3xl font-bold text-primary">Divulgación Pública de la Ciencia</h1>
      ...
      {!loading && !error && resultados.length > 0 && Object.keys(nodosHijo).length === 0 && (
        <p className="text-center text-gray-600 text-lg">No hay registros de "Divulgación Pública de la Ciencia". Total datos: {resultados.length}</p>
      )}
